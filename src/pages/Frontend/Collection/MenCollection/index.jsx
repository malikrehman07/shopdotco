  ...
  return (
    <main>
      ...
      <div className="container py-5">
        <Row>
          <Col span={24}>
            <Title level={1} className='text-center'>Men's Collection</Title>
          </Col>
        </Row>
              ...
              <Col xs={12} sm={12} md={12} lg={6} key={product.id}>
                <div className="card border-0" style={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <div style={{ cursor: 'pointer' }} onClick={() => navigate(`/collection/${product.category}/product/${product._id}`)} >
                    <img src={product.imageUrls?.[0]} alt={product.alt} style={{ width: "300px", height: "300px", objectFit: "cover" }} className="img-fluid rounded-4" />
                  ...
                  <div>
                    <Title level={4} className='mt-0'>{product.variants?.[0]?.specialPrice ? <>${product.variants?.[0]?.specialPrice} <strike style={{ color: "#c9c6c5" }}>${product.variants?.[0]?.price}</strike></> : <>${product.variants?.[0]?.price}</>}</Title>
